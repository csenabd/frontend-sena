...
import "../Principal/DetailPage.css"; // Importa el archivo CSS
import URL from "../../constants/api";
axios.defaults.headers.common[
  "Authorization"
] = `Bearer ${sessionStorage.getItem("token")}`;

function DetailPageSeno() {
  const { id } = useParams();
  ...
  useEffect(() => {
    axios
      .get(`${URL.API}/api/v1/ProyectoSeno/${id}`)
      .then((res) => {
  ...
  const sendCorreo = () => {
    axios
      .post(`${URL.API}/api/v1/ProyectoSeno/${id}/send-code`)
  ...
  const deleteProyecto = () => {
    axios
      .delete(`${URL.API}/api/v1/ProyectoSeno/${id}?confirmationCode=${codigo}`)
      .then((res) => {
        console.log(res);
  ...
  const handleObjetivoChange = (objetivoId, checked) => {
    axios
      .put(
        `${URL.API}/api/v1/ProyectoSeno/${id}/objetivo-especifico/${objetivoId}`,
        { finalizado: checked },
        {
          headers: {
...
const handleActividadChange = (objetivoId, actividadId, checked) => {
  ...
  axios
    .put(
      `${URL.API}/api/v1/ProyectoSeno/${id}/objetivo/${objetivoId}/actividad/${actividadId}`,
